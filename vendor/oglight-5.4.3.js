  ...
  updateJQuerySettings() {
    ...
    $(document).on("ajaxSuccess", function (event, xhr, settings) {
      if (
        ...
        let messageIDs = new URLSearchParams(settings.data.replace(/%5B%5D/g, "")).getAll(
          "messageIds",
        ) || [-1];
...
class MessageManager extends Manager {
  ...
  readMessagesData() {
    ...
    if (this.tabID == 20) {
      ...
      this.messageDB = Object.fromEntries(
        Object.entries(this.messageDB).filter(([id]) => messageIds.includes(id)),
      );
      this.saveMessagesDB();
